...
// 0.99 ceiling (near-identical images) was the old "100%" — the old default of 90%
// on that scale put the real decision boundary at cos ≈0.936, now the new 50%.
export const remapEmbed = (cos) => remapTri(cos, 0.45, 0.936, 0.99);
...
// ORB raw is the RANSAC inlier ratio: coincidental texture matches between unrelated
// images stay below ~0.15, true crops/rotations/flips land ~0.6-1.0 with a clean gap
// around ~0.5 in between (confirmed against example/) — so 0.5 (the old "100%"
// ceiling) is actually much closer to the real boundary than to "definitely the same".
export const remapOrb = (orb) => remapTri(orb, 0.15, 0.5, 1.0);
