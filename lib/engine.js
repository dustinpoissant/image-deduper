...
export function initCache() {
  if (!_cacheReady) _cacheReady = (async () => {
    ...
    // The SSCD tier landed after the images table existed in the wild, and
    // CREATE TABLE IF NOT EXISTS won't add columns to an existing one, so they're added
    // separately to upgrade a cache in place rather than forcing a full re-analysis.
    // sscdModel is tracked apart from `model` so the two tiers' caches invalidate
    // independently.
    //
    // Checked against the stored DDL rather than just attempting the ALTER and ignoring
    // the failure: the query bridge runs non-SELECT statements in the main process, which
    // logs the SqliteError there before this side ever sees a rejection — so a blind
    // retry prints "duplicate column name" to the terminal on every single launch.
    // SQLite rewrites sqlite_master.sql when a column is added, so it stays accurate.
    const ddl = (await sqlQ("SELECT sql FROM sqlite_master WHERE type='table' AND name='images'"))?.[0]?.sql || '';
    for (const [name, decl] of [['sscd', 'TEXT'], ['sscdModel', 'TEXT']]) {
      // \b stops `sscd` from matching inside `sscdModel`.
      if (!new RegExp(`\\b${name}\\b`).test(ddl)) await sqlQ(`ALTER TABLE images ADD COLUMN ${name} ${decl}`);
    }
  })();
  return _cacheReady;
}
...
export async function migrateExcludedPairs(oldHash, newHash) {
  ...
  const rows = await sqlQ(
    `SELECT pair FROM excluded_pairs WHERE pair LIKE ${esc(oldHash + '|%')} OR pair LIKE ${esc('%|' + oldHash)}`);
  const add = [];
  for (const r of rows) {
...
export async function gcCache() {
  ...
  await sqlQ(`DELETE FROM images WHERE hash NOT IN (${live})`);
  // orbcache keys are "hashA|hashB"; a row is dead if either side is gone.
  await sqlQ(
    `DELETE FROM orbcache WHERE substr(pair, 1, instr(pair, '|') - 1) NOT IN (${live}) ` +
    `OR substr(pair, instr(pair, '|') + 1) NOT IN (${live})`);
