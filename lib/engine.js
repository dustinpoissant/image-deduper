...
// Re-map each raw signal so "unrelated" ~ 0 and "duplicate" ~ 1.
// ORB raw is the RANSAC inlier ratio: true crops/rotations land ~0.4-1.0, while
// coincidental texture matches between different images stay below ~0.15. Requiring
// a real overlap (floor 0.15, full at 0.5) stops weak ORB edges from chaining clusters.
export const remapEmbed = (cos) => clamp01((cos - 0.45) / (0.99 - 0.45));
export const remapPhash = (sim) => clamp01((sim - 0.55) / (1 - 0.55));
export const remapOrb = (orb) => clamp01((orb - 0.15) / (0.5 - 0.15));

/* ---------- candidate pairs + clustering ---------- */

// Sparse pairwise signals, keeping only candidates above a low floor.
...
export function buildCandidatePairs(items) {
  ...
  for (let i = 0; i < n; i++) {
    for (let j = i + 1; j < n; j++) {
      ...
      if (Math.max(sEmbed, sPhash) >= 0.12) pairs.push({ i, j, sEmbed, sPhash, sOrb: null });
    }
  }
  return pairs;
}
